...
public class Product extends Model<Product> {
    ...
    public static final Product dao = Duang.duang(new Product());
    ...
    private transient ProductImageService productImageService = new ProductImageService();
    ...
    public Page<Product> getHotProductList(int maxResults, Integer pageNumber, Integer pageSize) {
        String select = "select * ";
        String sqlExceptSelect = " from Product  where isMarketable = true and isHot = true order by product.createDate asc ";
    ...
    public Product getProductBySn(String sn) {
        String sql = "select * from product t where t.productSn = ?";
        return dao.find(sql, sn).get(0);
    }

    public boolean delete(String id) {
        int count = Db.update("update product set isDelete = 1  where id = ?", id);
        return count >= 0 ? true : false;
    }

    public List<Product> getProductsByPacketId(String packetId) {
        return dao.find("select p.*,m.promotionPrice from product p join (select * from packetproductmap where packet_id=? ) m on p.id=m.product_id", packetId);
    }
}
