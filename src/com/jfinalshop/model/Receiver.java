...
public class Receiver extends Model<Receiver> {
    ...
    public List<Receiver> getReceiverList(String member_id) {
        String sql = "select * from receiver where member_id = ? order by isDefault desc";
        return dao.find(sql, member_id);
    }

    public Receiver getDefaultReceiverByMemberId(String member_id) {
        String sql = "select * from receiver where member_id = ? and isDefault=1";
        return dao.findFirst(sql, member_id);
    }
