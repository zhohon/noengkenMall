...
public class ProductController extends BaseAdminController<Product> {
    ...
    public void getProduct() {
        ...
        Page<Product> product = Product.dao.paginate(getParaToInt("page", 1), getParaToInt("rows", 10), select, sqlExceptSelect);
    ...
    // 保存
    @Before({ClearCache.class, ProductValidator.class})
    @J2CacheName(Product.class)
    public void save() {
        ...
        if (product.getBoolean("isMarketable")) {// 是否上架
            HtmlService.service.productContentBuildHtml(product, getRequest());
        }
    ...
    public void update() {
        ...
        // 生成静态页面
        if (product.getBoolean("isMarketable")) {// 是否上架
            HtmlService.service.productContentBuildHtml(persistent, getRequest());
        }
        redirect("/product/list");
    }
    ...
    public void forEdit() {
        ...
        if ("del".equals(oper)) {
            String idPara = getPara("id");
            String[] ids = idPara.split(",");
            for (String id : ids) {
                Product product = Product.dao.findById(id);
                List<OrderItem> orderItemList = product.getOrderItemList();
                if (orderItemList != null && orderItemList.size() > 0) {
