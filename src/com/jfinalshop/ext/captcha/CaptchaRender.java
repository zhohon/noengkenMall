package com.jfinalshop.ext.captcha;
...
/**
 * 验证码Render，这个验证码Render在构造函数里就已经创建好了随机码以及md5散列后的随机码。 调用方式如下： CaptchaRender
 * captchaRender = new CaptchaRender(); String md5RandonCode =
 * captchaRender.getMd5RandonCode(); 保存md5RandonCode到session、cookie或者其他地方
 * render(captchaRender); 基于JFinal的版本修改。
 * 
 */
public class CaptchaRender extends Render {
	...
	public CaptchaRender(int imgRandNumber) {
		...
		this.imgWidth = 16 * imgRandNumber + 15;
		this.imgHeight = 38;
	...
	public static boolean validate(String md5RandomCode, String inputRandomCode) {
		if (StrKit.isBlank(md5RandomCode)
				|| StrKit.isBlank(inputRandomCode))
			return false;
		try {
